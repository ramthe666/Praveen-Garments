...
/**
 * POST /api/customers/[id]/payments — record a customer payment against
 * outstanding dues (FIFO) with optional advance when enabled in settings.
 * Server recomputes everything; atomic (single RPC transaction).
 */
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const guard = await requirePermission('record_customer_payment')
