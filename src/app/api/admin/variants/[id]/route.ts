...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: existing } = await admin.from('product_variants').select('id').eq('id', id).maybeSingle()
  ...
  if (input.generate_barcode) {
    const { data: barcode, error: genError } = await admin.rpc('generate_barcode')
    if (genError || !barcode) {
  ...
  if (input.generate_qr) {
    const { data: qr, error: genError } = await admin.rpc('generate_qr_identifier')
    if (genError || !qr) {
